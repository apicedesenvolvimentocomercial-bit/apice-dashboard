 ...
 * busca de configuração + linha de chips + dois grupos (título FORA do card,
 * como no conteúdo real), na mesma coluna centralizada de 1180px.
 */
export default function ClinicSettingsLoading() {
  return (
    <div className="mx-auto w-full max-w-[1180px]">
      ...
      {/* grupos de seção: cabeçalho fora + card, separados por filete */}
      <div className="flex flex-col">
        {Array.from({ length: 2 }).map((_, i) => (
          <div key={i}>
            {i > 0 && <div className="my-8 h-px w-full bg-border" />}
            <div className="flex flex-col gap-3">
              <div className="px-0.5">
                <div className="senno-shimmer h-[18px] w-36 rounded" />
                <div className="senno-shimmer mt-2.5 h-3.5 w-4/5 max-w-md rounded" />
              </div>
              <div className="rounded-[14px] border border-border bg-card px-[26px] py-6 shadow-card max-sm:px-5">
