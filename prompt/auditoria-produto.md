...
| Item                                                                           | Local                                                                                      | Veredito                                                                                                                                                                                                                                                        |
| ------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `MarketingCampaign` (modelo inteiro)                                           | `prisma/schema.prisma:457`; `src/server/services/insights/rules/high-marketing-low-roi.ts` | **Órfão.** Sem CRUD, sem action, sem componente de UI. Só `Cost.campaignId` referencia. Porém a regra de insight de ROI e o KPI de CAC dependem de campanha → input nunca é preenchido → métrica morta. Decisão: ligar UI de campanhas **ou** remover o modelo. |
| `winLead` + `winLeadAction`                                                    | `src/server/services/lead-service.ts:11`; `src/server/actions/lead-actions.ts:407`         | Atalho "Ganhou" foi removido do drawer na Fase 3 das pipelines, mas a **server action continua exportada sem nenhum caller de UI**. Pior que lixo: é um caminho que **burla** o fluxo obrigatório Compareceu→Fechado (que exige Appointment ATTENDED). Remover. |
| `summarizeUnread`                                                              | `src/server/services/notification-service.ts:188`                                          | Dead code. Nunca importado. Remover.                                                                                                                                                                                                                            |
...
1. **Lembrete automático de agendamento** (WhatsApp/email, 48h + 24h, com
   confirmar/remarcar). O produto inteiro gira em torno de no-show
   (`Client.noShowWindowHours`, KPI `noShowRate`, etapa nativa "Cancelado") mas **só
   mede, nunca previne**. Maior ROI da lista.
