...
| D2  | **Ambiente p/ E2E**                      | E2E run      | **RESOLVIDA (2026-05-22):** banco **Neon** descartável. `.env.test` (gitignored) com pooled+direct do Neon. 15 migrations + seed aplicados (admin@senno.dev/admin123). Falta só `npx playwright install` (browsers).                                                                                                                            |
...
- **Seed E2E** (`prisma/seed-e2e.ts`): org + admin/staff + 2 clínicas (Alpha/Bravo) com owners (owner-a/owner-b @senno.dev / owner123) e um paciente em cada — base do teste de isolamento.
